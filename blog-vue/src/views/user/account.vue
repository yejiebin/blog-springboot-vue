  ...
  export default {
    ...
    data () {
      return {
        ...
        mail: '',           //邮箱
        updatePwdMailCode: '',  //验证码
        oldMailCode: '',        //原邮箱验证码
        newMail: '',            //新邮箱
        newMailCode: '',        //新邮箱验证码
        updatePwdSendFlag: false,       //发送验证码加载标志
        updateMailToNewSendFlag: false,
        updateMailToOldSendFlag: false
      }
    },
    created () {
      // this.mail = 'jiebinye@126.com';
      this.load();
    },
    methods: {
      load() {
      ...
      updateMailSendMailToOld() { //改绑邮箱发送验证码到旧邮箱
        this.updateMailToOldSendFlag = true;
        this.sendMail(this.mail);
      },
      updateMailSendMailToNew() { //改绑邮箱发送验证码到新邮箱
        this.updateMailToNewSendFlag = true;
        this.sendMail(this.newMail);
      },

      sendMail(mail) {  //发送邮箱验证码
      ...
      updatePwd() { //修改密码
        ...
        user.updatePassword(this.newPassword, this.updatePwdMailCode).then(res => {
          this.$message({
            message: '修改成功',
            type: 'success'
          });
      ...
      updateMail() {  //改绑邮箱
        ...
        if (this.oldMailCode.length <= 0 || this.newMailCode.length <= 0) {//
          this.$message({
            message: '请填写验证码',
            type: 'error'
          });
