    ...
    <div>
      <el-row>
        <el-menu :default-active="activeIndex" class="el-menu-demo" :router="true" mode="horizontal"
          ...
          <div style="width: 20%;float: left;margin: 10px 2% 0px 42%" class="hidden-xs-only">
        ...
        <el-dialog title="登录" :visible.sync="loginFormVisible" width="22%">
          <el-form :model="form">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      // },
      userLogin() {    //用户登录
        if (this.form.loginName.length < 1 || this.form.loginPwd < 1) {
          this.$message({
            type: 'error',
            message: '请输入用户名和密码',
          });
        ...
        user.login(this.form.loginName,this.form.loginPwd).then(res => {
      ...
      userRegister() {    //用户注册
        ...
        user.register(this.form.registerName, this.form.registerPwd, this.form.registerMail,
          this.form.registerMailCode).then(res => {
            this.$message({
              message: '注册成功',
              type: 'success'
            });
